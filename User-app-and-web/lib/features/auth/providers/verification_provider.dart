...
class VerificationProvider with ChangeNotifier {
  ...
  Future<ResponseModel> verifyToken(String? email) async {
    ...
    debugPrint(
        "-------------(VERIFY TOKEN)----UserInput $email and Verification Code $_verificationCode----------");
  ...
  Future<void> sendVerificationCode(
      BuildContext buildContext, ConfigModel config, String userInput,
      {required String type, required String fromPage}) async {
    _isLoading = true;
    ...
    if (AuthHelper.isCustomerVerificationEnable(config)) {
      if (type == VerificationType.email.name &&
        ...
        debugPrint(
            "--------------------(SEND VERIFICATION CODE Email)-----------$userInput , $type and $fromPage");
        ...
        debugPrint(
            "--------------------(SEND VERIFICATION CODE Firebase)-----------$userInput , $type and $fromPage");
        await firebaseVerifyPhoneNumber(buildContext, userInput, fromPage);
      } else if (type == VerificationType.phone.name &&
          AuthHelper.isPhoneVerificationEnable(config)) {
        debugPrint(
            "--------------------(SEND VERIFICATION CODE Phone)-----------$userInput , $type and $fromPage");
  ...
  Future<ResponseModel> checkEmail(
      BuildContext buildContext, String email, String fromPage) async {
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse =
        await authRepo.sendVerificationCode(email, VerificationType.email);
    ResponseModel responseModel;
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      responseModel = ResponseModel(true, apiResponse.response!.data["token"]);

      debugPrint("---------------API ${apiResponse.response?.data}");

      bool isReplaceRoute =
          GoRouter.of(Get.context!).routeInformationProvider.value.uri.path ==
              RouteHelper.verify;
      debugPrint(
          "----------------------(CHECK EMAIL)  ---- $email and $fromPage");
      ...
      if (isReplaceRoute) {
        ...
        RouteHelper.getVerifyRoute(buildContext, email, fromPage,
            action: RouteAction.pushReplacement);
      } else {
        debugPrint('---------HERE I AM ELSE------------');
        if (fromPage == FromPage.profile.name) {
          Navigator.pop(buildContext);
        }
        RouteHelper.getVerifyRoute(buildContext, email, fromPage,
      ...
      String? errorMessage = ApiCheckerHelper.getError(apiResponse)
          .errors
          ?.first
          .message
          .toString();
  ...
  Future<ResponseModel> checkPhone(
      BuildContext buildContext, String phone, String fromPage) async {
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse =
        await authRepo.sendVerificationCode(phone, VerificationType.phone);
    ResponseModel responseModel;
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      responseModel = ResponseModel(true, apiResponse.response!.data["token"]);
      ...
      bool isReplaceRoute =
          GoRouter.of(Get.context!).routeInformationProvider.value.uri.path ==
              RouteHelper.verify;
      debugPrint(
          "----------------------(CHECK Phone)  ---- $phone and $fromPage");

      if (isReplaceRoute) {
        if (fromPage == FromPage.profile.name) {
          Navigator.pop(buildContext);
        }
        RouteHelper.getVerifyRoute(buildContext, phone, fromPage,
            action: RouteAction.pushReplacement);
      } else {
        if (fromPage == FromPage.profile.name) {
          Navigator.pop(buildContext);
  ...
  Future<ResponseModel> verifyVerificationCode(BuildContext context,
    ...
    debugPrint(
        '-----------(VERIFY VERIFICATION CODE)-------$emailOrPhone and $_verificationCode');
  ...
  Future<(ResponseModel?, String?)> verifyPhoneForOtp(
    ...
    debugPrint(
        "------------------------(VERIFY PHONE FOR OTP)-----------------$phone");
    debugPrint(
        "------------------------(VERIFY PHONE FOR OTP)-----------------$_verificationCode");

    notifyListeners();
    ApiResponseModel apiResponse =
        await authRepo.verifyOtp(phone, _verificationCode);
    debugPrint(
        "-------------------------(API RESPONSE)---------------${apiResponse.response?.data}");
    ResponseModel? responseModel;
    String? token;
    String? tempToken;
    if (apiResponse.response != null &&
  ...
  Future<ResponseModel> verifyProfileInfo(BuildContext context,
      String userInput, String type, String? session) async {
    debugPrint(
        '-------------------(VERIFY PROFILE INFO)----------$userInput and $type');
    _isLoading = true;
    notifyListeners();
    ...
    debugPrint(
        '-------------------(VERIFY PROFILE INFO)----------$userInput and $type and $session');
    ApiResponseModel apiResponse = await authRepo.verifyProfileInfo(
        userInput, _verificationCode, type, session);
    debugPrint("----------------(API)--------------${apiResponse.toString()}");
    debugPrint(
        "--------------(API RESPONSE)--------------${apiResponse.error.toString()}");
    ResponseModel? responseModel;

    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      Map map = apiResponse.response!.data;
      debugPrint("------------(VERIFY PROFILE INFO)---------${map.toString()}");

      final ProfileProvider profileProvider =
          Provider.of<ProfileProvider>(Get.context!, listen: false);
      profileProvider.getUserInfo();
      showCustomSnackBar(
          apiResponse.response!.data['message'], isError: false, context);
      responseModel = ResponseModel(true, 'verification');
    } else {
      String? error =
  ...
  Future<void> firebaseVerifyPhoneNumber(
      BuildContext buildContext, String phoneNumber, String fromPage,
    ...
    FirebaseAuth.instance.verifyPhoneNumber(
      ...
      verificationFailed: (FirebaseAuthException e) {
        ...
        notifyListeners();
        showCustomSnackBar(
            getTranslated('${e.message}', buildContext), buildContext);
      },
      codeSent: (String vId, int? resendToken) {
        bool isReplaceRoute =
            GoRouter.of(buildContext).routeInformationProvider.value.uri.path ==
                RouteHelper.verify;
        debugPrint(
            '---------------------(IS REPLACE ROUTE)--------------------$isReplaceRoute');
        ...
        if (fromPage == FromPage.profile.name) {
          if (isReplaceRoute) {
            RouteHelper.getVerifyRoute(buildContext, phoneNumber, fromPage,
                session: vId, action: RouteAction.pushReplacement);
          } else {
            RouteHelper.getVerifyRoute(buildContext, phoneNumber, fromPage,
                session: vId, action: RouteAction.push);
          }
        } else {
          if (isReplaceRoute) {
            RouteHelper.getVerifyRoute(buildContext, phoneNumber,
