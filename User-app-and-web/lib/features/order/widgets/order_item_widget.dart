      ...
      return InkWell(
        onTap: () {
          RouteHelper.getOrderDetailsRoute(
              context, orderList![index].id, orderList![index],
    ...
    return Consumer<OrderProvider>(builder: (context, orderProvider, _) {
      return CustomButtonWidget(
        ...
        btnTxt: getTranslated(
            orderList![index].paymentMethod == 'stripe' &&
                    orderList![index].paymentStatus == 'unpaid'
                ? 'pay_now'
        ...
        backgroundColor: orderList![index].paymentMethod == 'stripe' &&
                orderList![index].paymentStatus == 'unpaid'
            ? Colors.black
            : null,
        onTap: () async {
          if (orderList![index].paymentMethod == 'stripe' &&
              orderList![index].paymentStatus == 'unpaid') {
            ...
            String url =
                ...
                "&&callback=${AppConstants.baseUrl}${RouteHelper.orderSuccessScreen}&&order_amount=${(orderList![index].orderAmount!).toStringAsFixed(2)}&&order_id=${orderList![index].id}";

            String webUrl =
                "customer_id=${profileProvider.userInfoModel?.id ?? ""}&&is_guest=$isGuest"
                "&&callback=$protocol//$hostname${kDebugMode ? ':$port' : ''}${RouteHelper.orderWebPayment}&&order_amount=${(orderList![index].orderAmount!).toStringAsFixed(2)}&&status=&&order_id=${orderList![index].id}";
          ...
          } else if (isRunning || orderList![index].orderType == 'pos') {
            RouteHelper.getOrderTrackingRoute(
                context, orderList![index].id, null,
                action: RouteAction.push);
          } else {
